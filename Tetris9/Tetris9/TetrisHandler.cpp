...
void TetrisHandler::ResourceBorderline(HANDLES h_Base)
{
	WCHAR name[40];
	for (int i = 0; i < BORDER_LINE_FRAME; i++)
	{
		wsprintf(name, L"image/borderline/border%d.bmp", i);
		h_Border[i].hbmp = (HBITMAP)LoadImage(NULL, name, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
		h_Border[i].dc = CreateCompatibleDC(h_Base.dc);
		GetObject(h_Border[i].hbmp, sizeof(BITMAP), &h_Border[i].bit);

		h_Border[i].hOldbmp = (HBITMAP)SelectObject(h_Border[i].dc, h_Border[i].hbmp);
	}
}
...
{
	u_Border.Frame = 0;
	u_Border.MaxFrame = BORDER_LINE_FRAME;

	ResourceBorderline(h_Base);		// 테두리라인 리소스 로드
...
void TetrisHandler::UpdateAll(HANDLES h_Base)
{
	switch (mode)
	{
	case PLAYER_COM:
...
void TetrisHandler::DrawPVC(HANDLES h_Base, RECT m_rtClient)
{
	...
	BitBlt(h_Base.dc, x, y, h_Pvc.bit.bmWidth, h_Pvc.bit.bmHeight, h_Pvc.dc, 0, 0, SRCCOPY);
}
...
void TetrisHandler::DrawBPvc(HANDLES h_Base, RECT m_rtClient)
{
	...
	DrawBorder(h_Base, m_rtClient, x, y);
	BitBlt(h_Base.dc, x, y, h_BPvc.bit.bmWidth, h_BPvc.bit.bmHeight, h_BPvc.dc, 0, 0, SRCCOPY);
}
...
void TetrisHandler::DrawBorder(HANDLES h_Base, RECT m_rtClient, int x, int y)
{
	StretchBlt(h_Base.dc, x-25, y-40, 550, 380, h_Border[u_Border.Frame].dc, 0, 0, 504, 326, SRCCOPY);
}
...
void TetrisHandler::UpdateBorder()
{
	if (UpdateCheck(u_Border, 40) == TRUE)
	{
		u_Border.Frame++;
		if (u_Border.Frame >= u_Border.MaxFrame)
			u_Border.Frame = 1;
	}
}

void TetrisHandler::Setmode(HANDLES h_Base)
{
	if (pvc->BackMenu() == TRUE)
	{
		mode = PLAYER_COM;
		pvc->~PlayerVsCom();
		pvc = new PlayerVsCom;
		...
		InitMode();
		InitTetrisResource(h_Base);
	}
	else if (pvp->BackMenu() == TRUE)
	{
		mode = PLAYER_COM;
		pvp->~PlayerVsPlayer();
		pvp = new PlayerVsPlayer;
...
void TetrisHandler::keyhandle(WPARAM wParam, HANDLES h_Base)
{
	switch (mode)
	{
	case PLAYER_COM:
